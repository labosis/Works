...
public class Assignment3 {
		...
		while(!loop){
			if(turns == true){
				...
				if(test == true){
					...
					centreArray = makemiss(launch,centreArray);
					leftArray2 =  pullinfomiss(launch,leftArray2);
				}
				turns = false;
			}
			else{
				...
				if(test == true){
					...
					System.out.println("missed! you hit the water...");
					centreArray2 = makemiss(launch,centreArray2);
					leftArray = pullinfomiss(launch,leftArray);
				}
				turns = true;
	...
	static char[][]pullinfohit (String Input, char[][] array){
		...
		array[lettercolumns][numberows] = 'H';
		return array;
	}
	...
	static char[][]makemove (String Input, char[][] array){
